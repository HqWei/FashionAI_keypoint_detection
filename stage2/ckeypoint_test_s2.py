    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            for j in tqdm(range(len(list_file))):
                ...
                img_full = misc.imread('../data/image_ori/image_test/r2testa/' + img_id)
...
img_clist = [['blouse',30000],
             ...
             ['trousers', 50000]]
pd_list = []
for listt in img_clist:
    test_data = demo(listt[0],listt[1])
    pd_list.append(test_data)

pd_all = pd.concat(pd_list)

pd_all.to_csv('result.csv', index=False, sep=',')